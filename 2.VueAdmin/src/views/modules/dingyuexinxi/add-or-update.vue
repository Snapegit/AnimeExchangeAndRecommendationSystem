<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="订阅单号" prop="dingyuedanhao">
					<el-input v-model="ruleForm.dingyuedanhao" placeholder="订阅单号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.dingyuedanhao" label="订阅单号" prop="dingyuedanhao">
					<el-input v-model="ruleForm.dingyuedanhao" placeholder="订阅单号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="订阅日期" prop="dingyueriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.dingyueriqi" 
						type="date"
						:readonly="ro.dingyueriqi"
						placeholder="订阅日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.dingyueriqi" label="订阅日期" prop="dingyueriqi">
					<el-input v-model="ruleForm.dingyueriqi" placeholder="订阅日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="信息备注" prop="xinxibeizhu">
					<el-input  v-model="ruleForm.xinxibeizhu" placeholder="信息备注" clearable  :readonly="ro.xinxibeizhu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="信息备注" prop="xinxibeizhu">
...
export default {
	...
	created() {
		this.ruleForm.dingyueriqi = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='dingyuedanhao'){
							this.ruleForm.dingyuedanhao = obj[o];
							this.ro.dingyuedanhao = true;
							continue;
						}
						...
						if(o=='dingyueriqi'){
							this.ruleForm.dingyueriqi = obj[o];
							this.ro.dingyueriqi = true;
							continue;
						}
						if(o=='xinxibeizhu'){
    ...
    // 提交
    onSubmit() {
		if(this.ruleForm.dingyuedanhao) {
			this.ruleForm.dingyuedanhao = String(this.ruleForm.dingyuedanhao)
		}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dingyuexinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dingyuexinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
