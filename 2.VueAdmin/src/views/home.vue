...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('yonghu','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"linear-gradient(rgb(93, 129, 166), rgb(67, 102, 140))","justifyContent":"center","display":"flex","height":"80px"}'>
		...
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('dongmanxinxi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"linear-gradient(rgb(93, 129, 166), rgb(67, 102, 140))","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{dongmanxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>动漫信息总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('dingyuexinxi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"linear-gradient(rgb(93, 129, 166), rgb(67, 102, 140))","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji7" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{dingyuexinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>订阅信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="dongmanxinxiChart1" class="echarts1" v-if="isAuth('dongmanxinxi','首页统计')"></div>
		<div id="dingyuexinxiChart1" class="echarts2" v-if="isAuth('dingyuexinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getyonghuCount();
		this.getdongmanxinxiCount();
		this.dongmanxinxiChat1();
		this.getdingyuexinxiCount();
		this.dingyuexinxiChat1();
	},
	methods:{
		// 词云
		...
		getdongmanxinxiCount() {
			this.$http({
				url: `dongmanxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.dongmanxinxiCount = data.data
				}
			})
		},
		dongmanxinxiChat1() {
        ...
        var dongmanxinxiChart1 = echarts.init(document.getElementById("dongmanxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                dongmanxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    dongmanxinxiChart1.resize();
                };
            }
        });
      })
		...
		getdingyuexinxiCount() {
			this.$http({
				url: `dingyuexinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.dingyuexinxiCount = data.data
				}
			})
		},
		dingyuexinxiChat1() {
        ...
        var dingyuexinxiChart1 = echarts.init(document.getElementById("dingyuexinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                dingyuexinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    dingyuexinxiChart1.resize();
                };
            }
        });
      })
