...
<div>
	...
	<div class="list-preview" :style='{"padding":"30px 10% 0","margin":"0px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"block","width":"100%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="category-3" :style='{"border":"0px solid #eee","padding":"0","boxShadow":"0px 0px 0px #eee","margin":"22px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"300px","float":"right","height":"auto","order":"3"}'>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"left","top":"50px","left":"10%","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"calc(80% - 340px)","position":"absolute","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>动漫名称：</div>
        <el-input v-model="formSearch.dongmanmingcheng" placeholder="动漫名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>动漫分类：</div>
        <el-select v-model="formSearch.dongmanfenlei" placeholder="请选择动漫分类" :clearable="true">
          <el-option v-for="(item, index) in dongmanfenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>所属地区：</div>
        <el-input v-model="formSearch.suoshudiqu" placeholder="所属地区" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>动漫作者：</div>
        <el-input v-model="formSearch.dongmanzuozhe" placeholder="动漫作者" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>发行年份：</div>
        <el-input v-model="formSearch.faxingnianfen" placeholder="发行年份" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('dongmanxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"0px","background":"#c5c5c5","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/dongmanxinxiAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"20px 0 0 2%","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"320px"}' class="img-box">
				<img @click.stop="imgPreView(item.fengmianxinxi)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.fengmianxinxi && item.fengmianxinxi.substr(0,4)=='http'" :src="item.fengmianxinxi.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.fengmianxinxi?item.fengmianxinxi.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.fengmianxinxi?item.fengmianxinxi.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">{{item.dongmanmingcheng}}</div>
				<div class="title">动漫作者:{{item.dongmanzuozhe}}</div>
				<div class="title">发行年份:{{item.faxingnianfen}}</div>
		      ...
		      <div class="desc">{{item.gushigaiyao}}</div>
		    </div>
		  </div>
		</div>
	</div>
	...
	<!-- 热门信息 -->
	<div class="hot" :style='{"border":"0px solid #e9e9e9","padding":"0","margin":"30px 0 0","background":"none","width":"300px","float":"right","height":"auto","order":"5"}'>
	  <div :style='{"padding":"0px 20px","color":"#fff","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231013/2284549a531d4e52a1e363e23ec760fc.jpg) no-repeat center center / auto 100%","width":"100%","fontSize":"18px","lineHeight":"43px"}'>推荐信息</div>
	  <div :style='{"padding":"0 10px","margin":"20px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","margin":"0 0 20px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"100px"}' @click="toDetail(item)">
	      <img :style='{"width":"49%","objectFit":"cover","float":"left","height":"100px"}' v-if="item.fengmianxinxi && item.fengmianxinxi.substr(0,4)=='http'" :src="item.fengmianxinxi.split(',')[0]" alt="">
	      <img :style='{"width":"49%","objectFit":"cover","float":"left","height":"100px"}' v-else :src="baseUrl + (item.fengmianxinxi?item.fengmianxinxi.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#666","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0px 0 0","width":"49%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.dongmanmingcheng}}</div>
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#666","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0px 0 0","width":"49%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.dongmanzuozhe}}</div>
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#666","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0px 0 0","width":"49%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.faxingnianfen}}</div>
	      <!--<div :style='{"padding":"0 10px","lineHeight":"32px","fontSize":"14px","color":"#999","textAlign":"right","display":"none"}'>2022-02-02</div>-->
	    </div>
	  </div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/dongmanfenlei/dongmanfenlei').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getHotList() {
        let autoSortUrl = "";
        autoSortUrl = "dongmanxinxi/autoSort";
        if(localStorage.getItem('Token')) {
            autoSortUrl = "dongmanxinxi/autoSort2";
        }
      ...
      getFenlei() {
		this.$http.get('dongmanfenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'dongmanfenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.dongmanmingcheng != '') searchWhere.dongmanmingcheng = '%' + this.formSearch.dongmanmingcheng + '%';
        if (this.formSearch.dongmanfenlei != '') searchWhere.dongmanfenlei = this.formSearch.dongmanfenlei;
        if (this.formSearch.suoshudiqu != '') searchWhere.suoshudiqu = '%' + this.formSearch.suoshudiqu + '%';
        if (this.formSearch.dongmanzuozhe != '') searchWhere.dongmanzuozhe = '%' + this.formSearch.dongmanzuozhe + '%';
        if (this.formSearch.faxingnianfen != '') searchWhere.faxingnianfen = '%' + this.formSearch.faxingnianfen + '%';
        if (this.curFenlei != '全部') searchWhere.dongmanfenlei = this.curFenlei;
        this.$http.get(`dongmanxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/dongmanxinxiDetail', query: params});
      },
