...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知名称" prop="tongzhimingcheng">
            <el-input  v-model="ruleForm.tongzhimingcheng" 
                placeholder="通知名称" clearable :readonly=" false  ||ro.tongzhimingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="动漫名称" prop="dongmanmingcheng">
            <el-input  v-model="ruleForm.dongmanmingcheng" 
                placeholder="动漫名称" clearable :readonly="true  ||ro.dongmanmingcheng"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知内容" prop="tongzhineirong">
            <el-input  v-model="ruleForm.tongzhineirong" 
                placeholder="通知内容" clearable :readonly=" false  ||ro.tongzhineirong"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知日期" prop="tongzhiriqi">
              <el-date-picker
				  :disabled="true  ||ro.tongzhiriqi"
...
<script>
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='tongzhimingcheng'){
              this.ruleForm.tongzhimingcheng = obj[o];
              this.ro.tongzhimingcheng = true;
              continue;
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`gengxintongzhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('gengxintongzhi/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(`gengxintongzhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
